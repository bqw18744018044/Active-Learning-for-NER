...
if active_config['active_strategy'] == 'mnlp':
    while (selected_texts is None) or len(selected_texts) < active_config['total_num']:
        ...
        tobe_selected_idxs, tobe_selected_scores = ActiveStrategy.mnlp(scores, unselected_texts, active_config['incremental_num'])
        dataPool.update(tobe_selected_idxs)
    logger.info("The final size of selected data is {}".format(len(selected_texts)))
else:
    # random sampling
    unselected_texts, unselected_labels = dataPool.get_unselected()
    tobe_selected_idx = ActiveStrategy.random_sampling(unselected_texts, active_config['total_num'] - active_config['initial_num'])
